...
setProfileData(profile) {
  // ✅ Default avatar
  if (profile.image === 'avatar.jpg' || !profile.image) {
    this.imgStr = routes.imageAsset + 'img/avatar.jpg';
  } else {
    // ✅ Always prepend full Laravel asset URL for storage
    this.imgStr = routes.storage + '/avatar/' + profile.image;
  }
  ...
  this.experienceLabel = this.experiences.find(el => el.id == exp)?.text || 'Experience';
  ...
  try {
    ...
    this.jobTypeName = this.jobTypes.find(el => el.id == jobType)?.text || 'Job Type';
  } catch (error) {
    console.log(error);
  }
}
,
    ...
    computed: {
      setImage() {
        return this.data ? routes.storage + '/storage/avatar/' + this.data.profile.image : routes.imageAsset + 'img/avatar.jpg';
      },
      setAbout() {
